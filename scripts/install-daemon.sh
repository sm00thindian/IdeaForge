...
echo "  launchctl kickstart -k $DOMAIN/$LABEL  # restart after config.toml changes"
echo "  ./scripts/uninstall-daemon.sh          # stop and remove"
echo ""
echo "After config.toml edits: kickstart above. After API key changes: re-run this installer, then kickstart."
echo "See README → LaunchAgent reload for the full table."